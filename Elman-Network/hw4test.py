...
def plotcomparison(trainList, testList, maxval, minval, savefolder):
    ...
    for node in trainList:
        ...
        y = y.reshape(100)
        plt.plot(x, y, color = 'blue', label='Gerçek Değer', linewidth=1)
    ...
    for node in trainList:
        ...
        y = minval + normalized_y*(maxval - minval)
        y = y.reshape(100)
        plt.plot(x, y, color = 'red', label='Tahmin edilen', linewidth=1)
        m = (node.val - node.prev.val)/0.99
        normalized_y = node.prev.val + (x - node.prev.index)*m
        y = minval + normalized_y*(maxval - minval)
        y = y.reshape(100)
        plt.plot(x, y, color = 'blue', label='Gerçekte olan', linewidth=1)
    ...
    plt.figure()
    plt.plot(trainingListPred_x, trainingListPred_y, color = 'red', linewidth=1)
    ...
    plt.figure()
    plt.plot(trainingList_x, trainingList_y, color = 'blue', linewidth=1)
    ...
    plt.figure()
    plt.plot(trainingListPred_x, trainingListPred_y, color = 'red', label='Tahmin edilen', linewidth=1)
    plt.plot(trainingList_x, trainingList_y, color = 'blue', label='Gerçekte olan', linewidth=1)
    ...
    plt.figure()
    plt.plot(testingListPred_x, testingListPred_y, color = 'red', linewidth=1)
    ...
    plt.figure()
    plt.plot(testingList_x, testingList_y, color = 'blue', label='Gerçekte olan', linewidth=1)
    ...
    plt.figure()
    plt.plot(testingListPred_x, testingListPred_y, color = 'red', label='Tahmin edilen', linewidth=1)
    plt.plot(testingList_x, testingList_y, color = 'blue', label='Gerçekte olan', linewidth=1)
...
def testNetwork(nodeList, network, prevx = 0):
    ...
    for node in nodeList:
        j = 0
        data = np.array([np.random.normal(0, 0.01)], dtype=float).reshape(dim, 1)
...
for i in range(train_init,train_last):
    y = YNode(i, yprev)
    yprev = y
...
testList = []
for i in range(train_last,test_last):
    y = YNode(i, yprev)
    yprev = y
...
a = np.zeros((5,1), dtype = float) + np.random.normal(0,2)
b = np.zeros((1,1), dtype = float) + np.random.normal(0,2)
c = np.zeros((5,1), dtype = float) + np.random.normal(0,2)
d = np.zeros((1,5), dtype = float) + np.random.normal(0,2)


ilk = np.dot(a,b) * np.dot(c,d)
iki = np.dot((np.dot(a,b)*c),d)
print("ilki", ilk)
print("ikinci", iki)
...
for i in range(25):
    ...
    network = generate_network(0.3, "sigmoid")
    ...
    print("x =", train_last,"-", test_last, " için test edildi ve ortalama yüzde %.7f hata elde edildi." % (100*result[0][0]))
    network.clear()
    # Yine eğitilen ağ ile testler yapılır ve sonuçlar 'Liste_Karistirilmadiginda' klasörüne kaydedilir.
    savefolder = os.path.join(__location__, 'Test//Liste_Karistirilmadiginda_Test' + str(i))
